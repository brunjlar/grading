...
initContext = do
    ...
    withConnection db $ \conn -> do
        ...
        execute_ conn "CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, image TEXT UNIQUE NOT NULL, archive BLOB NOT NULL)" 
        execute_ conn "CREATE TABLE IF NOT EXISTS submissions (id INTEGER PRIMARY KEY AUTOINCREMENT, userid TEXT NOT NULL REFERENCES users(id), taskid INTEGER NOT NULL REFERENCES tasks(id), time TEXT NOT NULL, archive BLOB NOT NULL, result TEXT NOT NULL)"
    return $ GC 
        { gcLock = lock
        , gcDB   = db
        }
